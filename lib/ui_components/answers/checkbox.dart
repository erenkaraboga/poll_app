...
class CheckboxListAnswer extends StatefulWidget {
  ...
  const CheckboxListAnswer({super.key, required this.question, required this.index});
  @override
  _CheckboxListAnswerState createState() => _CheckboxListAnswerState();
}

class _CheckboxListAnswerState extends State<CheckboxListAnswer> {
  var titleController = TextEditingController();
  List<String> lastOptions = [];
  List<bool> checkedValues = []; // Ekledik: Kutuların durumunu tutmak için bir liste
  List<String> list  = [];
  @override
  void initState() {
                              ...
                              children: [
                                Expanded(
                                  child: CheckboxListTile(
                                    ...
                                    value: checkedValues[index], // Değeri buradan al
                                    onChanged: (newValue) {
                                      setState(() {

                                        checkedValues[index] = newValue ?? false;
                                        if(checkedValues[index]){
                                          list.add(lastOptions[index]);
                                        }
                                        else{
                                          list.remove(lastOptions[index]);
                                        }
                                        var updated = widget.question.copyWith(answer:list );
